...
@unittest.mock.patch("builtins.open", new_callable=mock_open, read_data=TEST_DATA)
def test_score_by_file(mock_file, mock_translator, mock_output_handler):
    mock_translator.run_scorer.return_value = ['', '']
    mock_translator.batch_size = 1
    mock_file.return_value = TEST_DATA.splitlines()
    sockeye.translate.read_and_translate(translator=mock_translator, output_handler=mock_output_handler,
                                         chunk_size=2, source='/dev/null', target='/dev/null')

    # Ensure that our scorer has the correct input passed to it.
    mock_translator.make_scorer_input.assert_any_call(1, "Test file line 1", "Test file line 1")
    mock_translator.make_scorer_input.assert_any_call(2, "Test file line 2", "Test file line 2")

    # Ensure run_scorer gets called once.  Input here will be a dummy mocked result, so we'll ignore it.
    assert mock_translator.run_scorer.call_count == 1
